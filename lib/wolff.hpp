...
namespace wolff
{
        ...
        graph(unsigned D, unsigned L, bool is_torus = true) : D(D), L(L), is_torus(is_torus)
        {
            ...
            for (unsigned i = 0; i < nv; i++)
            {
                ...
                for (unsigned j = 0; j < D; j++)
                {
                    ...
                    if (touches_boundary(i, j) != 1) // We did not reach the end of the box
                    {
                        typename graph<vertex_prop, edge_prop>::halfedge f(this->vertices[i], this->vertices[n1]);
                        this->vertices[i].edges.push_back(f);
                    }
                    ...
                    if (touches_boundary(i, j) != -1) // We did not reach the end of the box
                    {
                        typename graph<vertex_prop, edge_prop>::halfedge b(this->vertices[i], this->vertices[n2]);
                        this->vertices[i].edges.push_back(b);
                    }
                }
            }
        };

        unsigned coordinate_projection(unsigned index, unsigned d)
        {
            return unsigned(index / pow(L, d)) % L;
        }

        int touches_boundary(unsigned index, unsigned d)
        {
            if (is_torus)
            {
                return 0;
            }
            unsigned projection = coordinate_projection(index, d);
            if (projection == L - 1) // touches right boundary
            {
                return 1;
            }
            else if (projection == 0) // touches left boundary
            {
                return -1;
            }
            else
    ...
    public:
        ...
        void init_spins(std::function<X_t(unsigned i)> init_function)
        {
            for (unsigned i = 0; i < nv; i++)
            {
                s[i] = init_function(i);
                          ...
                          measurement<R_t, X_t, G_t> &A)
        {
            ...
            while (!queue.empty())
            {
                ...
                if (!visited[i])
                { // don't reprocess anyone we've already visited!
                    visited[i] = true;
        ...
        void finite_states_init()
        {
            ...
            for (unsigned i = 0; i < WOLFF_FINITE_STATES_N; i++)
            {
                ...
                {
#ifndef WOLFF_SITE_DEPENDENCE
                    Bp[i][j] = 1.0 - exp(-(B(X_t(i)) - B(X_t(j))) / T);
#else
                    typename G_t::vertex v{};
                    Bp[i][j] = 1.0 - exp(-(B(v, X_t(i)) - B(v, X_t(j))) / T);
#endif
                }
            }
#endif
